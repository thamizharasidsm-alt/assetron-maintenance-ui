(function () {
  ...
  function fields(excludeId) {
    return [
      ...
      { key: "categoryId", label: "Category", type: "select", options: departmentOptions() },
      ...
      { key: "frequencyDays", label: "Default PM Frequency (Days)", type: "number", required: true },
  ...
  const columns = [
    ...
    { key: "categoryId", label: "Category" }, { key: "parentEquipmentId", label: "Parent" }, { key: "locationId", label: "Location" },
    { key: "frequencyDays", label: "Default Frequency" }, { key: "status", label: "Status" },
  ];
  ...
  window.Pages["/masters/equipment"] = function (container) {
    ...
    function render() {
      ...
      container.querySelectorAll("tbody tr[data-id]").forEach((tr) => {
        ...
        cells[3].textContent = eq.categoryId ? (Store.find("departments", eq.categoryId)?.name || "—") : "—";
        cells[4].textContent = eq.parentEquipmentId ? (Store.find("equipment", eq.parentEquipmentId)?.name || "—") : "—";
        cells[5].textContent = eq.locationId ? Store.locationPath(eq.locationId).join(" › ") : "—";
        cells[6].textContent = (eq.frequencyDays || eq.frequencyDays === 0) ? `${eq.frequencyDays} day(s)` : "—";
      });
      wire();
    }
