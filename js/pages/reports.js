(function () {
  ...
  window.Pages["/reports/equipment-history"] = function (container) {
    ...
    function render() {
      ...
      container.innerHTML = `
        <div class="ac-page-header"><div><h1>${icon("clock", 22)} Equipment History</h1><p class="ac-page-subtitle">All Work Order and Calibration transactions recorded against a selected equipment.</p></div></div>
  ...
  window.Pages["/reports/range"] = function (container) {
    let from = todayStr(-90), to = todayStr(), modules = { workOrders: true, calibrations: true }, categoryId = "";
    render();
    function render() {
      const rows = buildRows();
      const categoryOptions = Store.all("spareCategories").map((c) => `<option value="${c.id}" ${c.id === categoryId ? "selected" : ""}>${c.name}</option>`).join("");
      container.innerHTML = `
        <div class="ac-page-header"><div><h1>${icon("barchart", 22)} Range Report</h1><p class="ac-page-subtitle">Transactions performed within a date range, across Work Order and/or Calibration. Optionally filter to transactions that used a spare from a given category.</p></div>
        ...
        <div class="ac-table-wrap"><table class="ac-table">
          <thead><tr><th>Doc No.</th><th>Module</th><th>Equipment</th><th>Performed Date</th><th>Performed By</th><th>Status</th></tr></thead>
          <tbody>${rows.length === 0 ? `<tr><td colspan="6"><div class="ac-empty-inline">No transactions in this range.</div></td></tr>` : rows.map((r) => `
            <tr><td><strong>${r.docNumber}</strong></td><td>${badge(r.module, "info")}</td><td>${eqName(r.equipmentId)}</td><td>${fmtDate(r.performedDate)}</td><td>${empName(r.performedById)}</td><td>${badge(r.status === "PendingApproval" ? "Pending" : r.status)}</td></tr>`).join("")}</tbody>
        </table></div>`;
      ...
      container.querySelector("#btn-export").addEventListener("click", () => {
        exportCsv("range-report.csv", rows.map((r) => [r.docNumber, r.module, eqName(r.equipmentId), r.performedDate, empName(r.performedById), r.status]), ["Doc No", "Module", "Equipment", "Performed Date", "Performed By", "Status"]);
      });
    }
